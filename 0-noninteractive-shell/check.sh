...
if stage_require_ready "00-shell"; then
...
if [ -f "$STAGE_ROOT/00-shell/init.sh" ] && \
   [ -f "$STAGE_ROOT/00-shell/env.sh" ] && \
   [ -f "$STAGE_ROOT/00-shell/path.sh" ] && \
   [ -f "$STAGE_ROOT/00-shell/require.sh" ] && \
   bash -n \
     "$STAGE_ROOT/00-shell/init.sh" \
     "$STAGE_ROOT/00-shell/env.sh" \
     "$STAGE_ROOT/00-shell/path.sh" \
     "$STAGE_ROOT/00-shell/require.sh"; then
...
if [ -f "$STAGE_ROOT/00-shell/init.sh" ] && grep -q 'source_required' "$STAGE_ROOT/00-shell/init.sh"; then
