...
domain_load_env() {
  ...
  export DOTS_HOME XDG_CONFIG_HOME XDG_DATA_HOME XDG_OPT_HOME XDG_STATE_HOME XDG_CACHE_HOME TOOL_PATH_HOME
...
domain_install_from_generated() {
  ...
  if [ "${DOMAIN_DRY_RUN:-0}" -ne 1 ]; then
    mkdir -p "$DOMAIN_STATE" "$DOMAIN_CACHE" "$DOMAIN_PREFIX" "$DOMAIN_BIN_HOME" "$DOMAIN_SHARE_HOME" "$TOOL_PATH_HOME"
  fi
